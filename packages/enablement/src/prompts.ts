...
/**
 * Who this is for and who they're talking to — shared by every prompt this
 * package builds, full analysis or verses alone. Kept as one fragment so the
 * two prompts never drift into describing the seeker differently.
 */
...
/**
 * The sidebar's instructions — the full analysis: verses, discussion points,
 * and a read on the seeker.
 *
...
/**
 * The lighter pass: verses only, meant to run automatically after every new
 * seeker message rather than on request. Shares the same seeker framing and
 * the same hard rules as the full prompt — only the "what you produce"
 * section is narrower, because that's the only part of the job this call
 * actually does.
 */
export function buildVersesPrompt(profile: DoctrineProfile = ECUMENICAL_PROFILE): string {
  return `${preamble()}

## What you produce

You are producing scripture suggestions only, for a lighter pass that runs automatically after each new seeker message. Nothing else the full sidebar produces — no discussion points, no read on the seeker — is your job here.
